...
public:
    constexpr static Time from_timestamp(i32 year, u8 month, u8 day, u8 hour, u8 minute, u8 second, u16 millisecond)
    {
    ...
    static Time from_ticks(clock_t, time_t);
    ...
    constexpr static Time min() { return Time(-_I64_MAX - 1LL, 0); };
    constexpr static Time zero() { return Time(0, 0); };
    constexpr static Time max() { return Time(_I64_MAX, 999'999'999); };

#ifndef KERNEL
    static Time now_realtime();
    static Time now_realtime_coarse();
    static Time now_monotonic();
    static Time now_monotonic_coarse();
#endif
    ...
    bool is_zero() const { return (m_seconds == 0) && (m_nanoseconds == 0); }
    bool is_negative() const { return m_seconds < 0; }
